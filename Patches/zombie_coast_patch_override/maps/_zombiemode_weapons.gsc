...
{
	if ( level.script == "zombie_coast" ) // dumbasses will try to play on another map
	{
		// setup
		if ( patch\_weighted_setup::is_weighted_setup_active() )
		{
			if ( patch\_weighted_setup::should_give_weighted_weapon() )
			{
				if ( !level.setup_complete )
				{
					return patch\_weighted_setup::get_next_weighted_weapon( player );
				}
			} 
		}

		// trades
		if ( patch\_weighted_trades::is_weighted_trades_active() )
		{
			weighted_trade_weapon = patch\_weighted_trades::get_next_weighted_weapon();

			if ( patch\_weighted_trades::should_give_weighted_weapon( weighted_trade_weapon, player ) )
			{
				patch\_weighted_trades::reset_variables( weighted_trade_weapon );
				patch\_weighted_trades::recalculate_average( weighted_trade_weapon );
				return weighted_trade_weapon[level._PATCH_WEAPON_NAME];
			}
			else
			{
				patch\_weighted_trades::on_box_hit();
			}
		}
	}
	...
	filtered_weapon = filtered[RandomInt( filtered.size )];
